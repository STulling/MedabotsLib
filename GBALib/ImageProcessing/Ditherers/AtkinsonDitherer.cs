...
{
    internal class AtkinsonDitherer : Ditherer
    {
        ...
        private void DistributeError(float[][] errorMatrix, int x, int y, float rError, float gError, float bError)
        {
            ...
            if (x + 1 < width)
            {
                errorMatrix[y][x * 3 + 3] += rError * 1 / 8;
                errorMatrix[y][x * 3 + 4] += gError * 1 / 8;
                errorMatrix[y][x * 3 + 5] += bError * 1 / 8;
            }

            if (x + 2 < width)
            {
                errorMatrix[y][x * 3 + 6] += rError * 1 / 8;
                errorMatrix[y][x * 3 + 7] += gError * 1 / 8;
                errorMatrix[y][x * 3 + 8] += bError * 1 / 8;
            }
            ...
            if (y + 1 < height)
            {
                if (x - 1 >= 0)
                {
                    errorMatrix[y + 1][x * 3 - 3] += rError * 1 / 8;
                    errorMatrix[y + 1][x * 3 - 2] += gError * 1 / 8;
                    errorMatrix[y + 1][x * 3 - 1] += bError * 1 / 8;
                }

                errorMatrix[y + 1][x * 3] += rError * 1 / 8;
                errorMatrix[y + 1][x * 3 + 1] += gError * 1 / 8;
                errorMatrix[y + 1][x * 3 + 2] += bError * 1 / 8;

                if (x + 1 < width)
                {
                    errorMatrix[y + 1][x * 3 + 3] += rError * 1 / 8;
                    errorMatrix[y + 1][x * 3 + 4] += gError * 1 / 8;
                    errorMatrix[y + 1][x * 3 + 5] += bError * 1 / 8;
                }
            }

            if (y + 2 < height)
            {
                errorMatrix[y + 2][x * 3] += rError * 1 / 8;
                errorMatrix[y + 2][x * 3 + 1] += gError * 1 / 8;
                errorMatrix[y + 2][x * 3 + 2] += bError * 1 / 8;
            }
        }
    }
}