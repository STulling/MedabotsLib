...
{
    internal class Sierra2Ditherer : Ditherer
    {
        ...
        private void DistributeError(float[][] errorMatrix, int x, int y, float rError, float gError, float bError)
        {
            ...
            if (x + 1 < width)
            {
                errorMatrix[y][x * 3 + 3] += rError * 4 / 16;
                errorMatrix[y][x * 3 + 4] += gError * 4 / 16;
                errorMatrix[y][x * 3 + 5] += bError * 4 / 16;
            }
            ...
            if (y + 1 < height)
            {
                if (x - 1 >= 0)
                {
                    errorMatrix[y + 1][x * 3 - 3] += rError * 2 / 16;
                    errorMatrix[y + 1][x * 3 - 2] += gError * 2 / 16;
                    errorMatrix[y + 1][x * 3 - 1] += bError * 2 / 16;
                }

                errorMatrix[y + 1][x * 3] += rError * 3 / 16;
                errorMatrix[y + 1][x * 3 + 1] += gError * 3 / 16;
                errorMatrix[y + 1][x * 3 + 2] += bError * 3 / 16;
